...
public final class BuildingState extends State {
    ...
    public State run() {
        ...
        for (final Player player : model.getPlayers()) {
            ...
            player.getDungeon().getSpells().clear();
        }
        ...
        //Player's list of locked bids by spells is cleared
        //Player's Map of locked rooms by spells is cleared
        for (final Player player : model.getPlayers()) {
            player.clearSpelledBids();
            player.getDungeon().getGraph().clearBlockedRooms();
        }
    ...
    private boolean handleSeason() {
        ...
        if (!evaluateBids(activateTriggers(model.getAvailableSpells(), biddingSquare))) {
            return false;
        }
    ...
    /**
     * Checks all triggering types of bids of all drawn spells for this round and
     * adds them to the spell list of the corresponding bid, placed in the biddingSquare
     *
     * @param availableSpells all drawn spells for this round
     * @param biddingSquare already initialized Map, representing the bidding square
     * @return the updated bidding square
     */
    public Map<BidType, List<Bid>> activateTriggers(final Iterable<Spell> availableSpells,
                                                     final Map<BidType, List<Bid>> biddingSquare) {
        for (final Spell spell : availableSpells) {
            final Spell currentSpell = spell;
            final int slot = currentSpell.getSlot();
            final BidType bidType = currentSpell.getBidType();
            final List<Bid> bidList = biddingSquare.get(bidType);
            if (bidList != null
                    && biddingSquare.get(bidType).size() >= slot) {
                biddingSquare.get(bidType).get(slot - 1).setTriggered(true);
                biddingSquare.get(bidType).get(slot - 1).setSpell(Optional.of(currentSpell));
            }
        }
