...
public class DigTunnelCommand extends PlayerCommand {
     ...
     */
    public static boolean violatesTunnelBuildingRestrictions(
            final Coordinate coordinate, final TunnelGraph graph, final int sideLength) {
        // Check out of bounds
    ...
    public ActionResult run(final Model model, final ConnectionWrapper connection) {
        ...
        if (DigTunnelCommand.violatesTunnelBuildingRestrictions(this.coordinate,
                graph, model.getDungeonSideLength())
                || !isConnected(this.coordinate, graph)) {
        ...
        spellFound(model, player.getDungeon(), this.coordinate, connection, player);
        if (player.getNumTunnelDigsAllowed() <= 0) {
    ...
    /**
     *Increases player's counter spells if found
     */
    public void spellFound(final Model model, final Dungeon dungeon,
                           final Coordinate coordinate, final ConnectionWrapper connection,
                           final Player player) {

        if (model.getRandom().nextInt(model.getDungeonSideLength() * model.getDungeonSideLength())
                < coordinate.posX() * coordinate.posY()) {
            dungeon.setCounterSpells(dungeon.getCounterSpells() + 1);
